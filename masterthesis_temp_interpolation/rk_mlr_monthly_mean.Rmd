...
```{r}
# Read CSV (columns: month, cv_rmse, cv_mae, cv_bias)
...
p_cv <- ggplot(cv_metrics_long,
  ...
  labs(
    title    = "Monthly Evaluation of Regression-Kriging LOOCV Metrics",
    subtitle = "Hourly RMSE and MAE values from LOOCV of models predicting monthly mean temperature",
    x        = "Month",
    y        = "Error Metric [°C]",
    color    = "Metrics",
...
# Optional: a separate plot for monthly bias (often centered near 0, separate y-scale helps)
p_bias <- ggplot(cv_metrics %>%
                   mutate(month_date = as.Date(paste0(month, "-01"))) %>%
                   arrange(month_date),
                 aes(x = month_date, y = cv_bias, group = 1)) +
  geom_hline(yintercept = 0, linetype = "dotted") +
  geom_line() +
  geom_point() +
  scale_x_date(date_breaks = "1 month", date_labels = "%Y-%m") +
  labs(
    title = "Monthly LOOCV Bias",
    x = "Month",
    y = "Bias [°C]"
  ) +
  theme_minimal(base_size = 14) +
...
# ggsave("../plots/cv_metrics_monthly_bias.png", plot = p_bias, width = 10, height = 4, dpi = 300)
...
# cv_metrics: columns month, cv_rmse, cv_mae, cv_bias
df_long <- cv_metrics %>%
  mutate(month_date = as.Date(paste0(month, "-01"))) %>%
  pivot_longer(c(cv_rmse, cv_mae), names_to = "metric", values_to = "value") %>%
  arrange(month_date)

# Okabe–Ito palette (color-blind friendly)
cols <- c(cv_rmse = "#0072B2",  # blue
          cv_mae  = "#D55E00")  # orange

# last points per series for direct labels
lab_data <- df_long %>% group_by(metric) %>% slice_max(month_date, n = 1) %>% ungroup()

# summer shading (June–Aug); adjust if your climate story differs
yr <- year(min(df_long$month_date, na.rm = TRUE))
summer_start <- as.Date(sprintf("%d-06-01", yr))
summer_end   <- as.Date(sprintf("%d-08-31", yr))

p_cv <- ggplot(df_long, aes(month_date, value, color = metric, group = metric)) +
  # season band (draw first)
  annotate("rect", xmin = summer_start, xmax = summer_end,
           ymin = -Inf, ymax = Inf, alpha = 0.06, fill = "grey50") +
  geom_line(linewidth = 1.2) +
  geom_point(size = 2, stroke = 0.2) +
  geom_hline(yintercept = 0, linetype = "dotted") +
  # direct labels near last month
  geom_label_repel(
    data = lab_data,
    aes(label = ifelse(metric == "cv_rmse", "RMSE", "MAE")),
    show.legend = FALSE, fill = "white", size = 4, label.r = unit(2, "pt"),
    min.segment.length = 0, nudge_x = 15
  ) +
  scale_color_manual(values = cols, labels = c(cv_mae = "Mean Absolute Error (MAE)",
                                               cv_rmse = "Root Mean Square Error (RMSE)")) +
  scale_x_date(date_breaks = "1 month", date_labels = "%Y-%m", expand = expansion(mult = c(.01,.05))) +
  labs(
    title    = "Monthly Evaluation of Regression-Kriging LOOCV Metrics",
    subtitle = "RMSE and MAE for monthly mean temperature (shaded = Jun–Aug)",
    x = "Month",
    y = "Error [°C]",
    color = NULL,
    caption = "LOOCV = leave-one-out cross-validation"
...
p_cv
# ggsave("cv_metrics_monthly_rmse_mae.png", p_cv, width = 11, height = 6, dpi = 300)
...
rk_metrics <- read.csv("../rk_results_week_01/rk_metrics.csv")
...
lm_metrics <- read.csv("../rk_results/lm_model_performance_summary.csv")
