...
p_vif_comp <- ggplot(vif_df, aes(x = covariate, y = VIF,
                                 fill = covariate, alpha = model)) +
  geom_col(
  ...
  annotate("text", x = 0.5, y = 5.5,
           label = "left: No filter; right: Gaussian filter\nVIF > 5 selected multicollinearity threshold",
  ...
  scale_fill_manual(
    values = c(
      "Building height" = "darkorange3",
      "Canopy height"   = "darkolivegreen",
      "Elevation"       = "burlywood2"
    ),
    guide = "none"
  ) +
  scale_alpha_manual(
    values = c("No filter" = 1.0, "Gaussian filter" = 0.75)
  ) +
  guides(alpha = "none") +
