...
```{r}
ggplot(temp_clean_station_hm, aes(x = hour, y = entity_id, fill = hm_temp)) +
  geom_tile() +
  scale_fill_viridis_c(option = "plasma") +
  labs(title = "Hourly Mean Temperature (Jan 2023)",
       x = "Date & Hour",
       y = "Station",
       fill = "°C") +
  theme_minimal() +
  theme(axis.text.y = element_text(size = 6))  # shrink labels if 29 stations
```
...
ggplot(temp_by_hour, aes(x = factor(hour_of_day), y = hm_temp, fill = hour_of_day)) +
  geom_boxplot(fill = "#FFA500", outlier.alpha = 0.3) +
  labs(
