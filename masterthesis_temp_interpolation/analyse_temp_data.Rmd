...
```{r}
# Read temperature data for the entire year of 2023
temp_outlier <- sf::read_sf('../data/temp_HD/sensor_data_year_2023.geojson')

station_data <- subset(temp_outlier, stationname == "MeteoHelix_09_Neuenheim_Bürgeramt")

# Palette (adjust as you like)
box_fill  <- "#1da1f2"
box_line  <- "#0f4c81"
out_color <- "#0f4c81"
text_col  <- "#071e3d"
...
station_data %>%
  ...
  geom_boxplot(
    fill = box_fill, color = box_line,
    outlier.shape = 18, outlier.size = 1.5, outlier.alpha = 1,
    outlier.colour = out_color
  ) +
  # 3-line summary above each box: Count, Mean, SD
  stat_summary(
    fun.data = function(y) {
      y <- y[!is.na(y)]
      ...
      iqr <- q3 - q1
      upper <- q3 + 1.5 * iqr
      pad <- 0.2 * diff(range(y)) + 1e-9  # a little headroom
    ...
    vjust = 0, size = 3.6, colour = text_col
  ) +
  labs(
    title = "Monthly temperature distribution — MeteoHelix_09_Neuenheim_Bürgeramt\nInactive since May 8, 2023",
    x = "Month",
    y = "Temperature (°C)"
  ) +
  # nicer month labels + extra top room for the summary text
