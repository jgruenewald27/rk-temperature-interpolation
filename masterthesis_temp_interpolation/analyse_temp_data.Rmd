...
# Plot daily lines + 7-day smooth, faceted by ordered entity_id
p_temp <- ggplot(daily, aes(x = date, y = tmean)) +
  geom_line(alpha = 0.25, linewidth = 0.3) +
  geom_line(aes(y = tmean7), linewidth = 0.6) +
  facet_wrap(~ entity_id, ncol = 5, scales = "fixed") +
  scale_x_date(date_breaks = "3 months", date_labels = "%b",
               limits = as.Date(c("2023-01-01","2023-12-31"))) +
  labs(title = "Temperature 2023 (ordered by unique sensor ID)",
...
p_dist <- ggplot(counts_5C_clean, aes(x = bin, y = n)) +
  geom_col(fill = "#87cefa", color = "black", alpha = 0.7) +
...
p_outlier <- station_data %>%
  ...
  geom_violin(fill = "#87cefa", color = "black", trim = FALSE,
  ...
  stat_summary(
    ...
    aes(label = after_stat(label)), size = 2.6, colour = text_col
  ) +
