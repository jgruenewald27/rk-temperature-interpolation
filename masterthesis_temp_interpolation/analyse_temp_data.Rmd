...
# Summarise spatial distribution per hour
temp_hourly_sum <- temp_july_12 %>%
  group_by(hour) %>%
  summarise(
    median_temp = median(hm_temp, na.rm = TRUE),
    q10 = quantile(hm_temp, 0.10, na.rm = TRUE),
    q90 = quantile(hm_temp, 0.90, na.rm = TRUE),
    .groups = "drop"
  )

# Create daily break positions (midnight)
day_lines <- seq(
  from = as.POSIXct("2023-07-01 00:00:00", tz = "Europe/Berlin"),
  to   = as.POSIXct("2023-07-13 00:00:00", tz = "Europe/Berlin"),
  by   = "1 day"
)

temp_time_series <- ggplot(temp_hourly_sum, aes(x = hour)) +
  geom_vline(xintercept = day_lines, colour = "grey80", linewidth = 0.4,
             alpha = 0.25) +
  geom_ribbon(aes(ymin = q10, ymax = q90), alpha = 0.25) +
  geom_line(aes(y = median_temp), linewidth = 0.9) +
  ...
  theme(
    ...
    panel.grid.major.y = element_line(colour = alpha("grey80", 0.25),
                                      linewidth = 0.4),
...
ggsave(
  ...
  plot = p_temp, width = 15, height = 10, units = "in", dpi = 300
)
```
...
p_outlier <- station_data %>%
  ...
  geom_violin(fill = "#FCE9B4", color = "black", trim = FALSE,
  ...
  stat_summary(
    ...
    aes(label = after_stat(label)), size = 3.2
  ) +
  labs(x = "Month", y = "Temperature [°C]") +
...
# Save mean plot
ggsave("../plots/monthly_mean_temperatures_2023.png", p_mean, width = 8, height = 4, dpi = 300)
...
```{r}
all_data %>%
  arrange(entity_id, dateobserved) %>%
  group_by(entity_id) %>%
  mutate(dt_mins = as.numeric(difftime(dateobserved, lag(dateobserved), units = "mins"))) %>%
  count(entity_id, dt_mins, sort = TRUE)

hd_data <- all_data %>%
  filter(entity_id == "hd:DE_Heidelberg_69120_34:WeatherObserved") %>%
  arrange(stationname)
