...
timestamps <- unique(temp_clean_station_hm$chour)[1:168]  # Test with first 3 unique chour timestamps
...
# Add date (day only) and hour-of-day
cv_metrics <- cv_metrics %>%
  mutate(
    date = as.Date(datetime_calc),
    hour = hour(datetime_calc)   # optional, if you want x-axis to be 0–23
  )
...
ggplot(df_plot, aes(x = hour, y = value, color = metric, group = metric)) +
  geom_line(linewidth = 1) +
  geom_point(size = 1.8) +
  facet_wrap(~ date, ncol = 3, scales = "fixed") +   # gleiche y-Achse
  scale_color_manual(values = c("cv_rmse" = "#1f77b4",
                                "cv_mae"  = "#ff7f0e"),
...
rk_metrics_long <- rk_metrics %>%
  pivot_longer(cols = c(rk_rmse, rk_mae, rk_bias),
               names_to = "metric",
               values_to = "value")

# Plot metrics over timestamp
ggplot(rk_metrics_long, aes(x = timestamp, y = value, color = metric)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  theme_minimal(base_size = 14) +
  labs(title = "Regression-Kriging Metrics over Time",
       x = "Timestamp",
       y = "Value",
       color = "Metric") +
  theme(legend.position = "top")
