...
# Read cv metrics
cv_metrics <- read.csv("../rk_results_07-2023/cross_validation_metrics.csv",
                       stringsAsFactors = FALSE)
...
for (i in seq_len(n_pages)) {
  p_cv <- ggplot(df_plot, aes(x = hour, y = value, color = metric, group = metric)) +
    ...
    labs(
      title    = sprintf("Daily Evaluation of the Regression-Kriging Leave-One-Out Cross-Validation Metrics (Page %d/%d)", i, n_pages),
