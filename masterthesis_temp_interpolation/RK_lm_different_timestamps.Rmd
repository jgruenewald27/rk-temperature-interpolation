...
Compare the Stadtklima-Analyse with my RK prediction surface
...
river <- st_read("../data/waterways/waterways_HD.gpkg")                  # Neckar river polyline(s)
landuse <- st_read("../data/DE522L1_HEIDELBERG_UA2018_v013/Data/DE522L1_HEIDELBERG_UA2018_v013.gpkg")

# Sanity check: model must have a valid CRS
if (is.na(crs(mod))) stop("`mod` has no CRS. Set it first, e.g. crs(mod) <- 'EPSG:xxxx'.")

# Get the model CRS as a proj string (works for both terra + sf)
crs_mod <- crs(mod, proj = TRUE)
crs(ref5) <- "EPSG:32632"

# Project the 5 m reference to the model's CRS *and grid*
ref30 <- project(ref5, mod, method = "bilinear")

# Reproject vectors to the model CRS
crs_mod <- crs(mod, proj = TRUE)
hd_bounds <- st_transform(hd_bounds, crs = crs_mod)
river     <- st_transform(river,     crs = crs_mod)
landuse   <- st_transform(landuse,   crs = crs_mod)

# Clip/mask AFTER projection
...
```{r}
# If your objects are single-hour already, keep these as-is:
mod_h   <- mod          # or: mod_h <- mod[[idx_mod_14]]
ref30_h <- ref30        # or: ref30_h <- ref30[[idx_mod_14]]

# 1) Ensure identical NA mask
...
# 3) Metrics
pearson_r <- cor(df$rk_pred, df$`2_22_074_ist_t14_rev01`, method = "pearson")
spearman  <- cor(df$rk_pred, df$`2_22_074_ist_t14_rev01`, method = "spearman")
bias <- mean(df$rk_pred - df$`2_22_074_ist_t14_rev01`, na.rm = TRUE)
mae  <- mean(abs(df$rk_pred - df$`2_22_074_ist_t14_rev01`), na.rm = TRUE)
rmse <- sqrt(mean((df$rk_pred - df$`2_22_074_ist_t14_rev01`)^2, na.rm = TRUE))

print(c(pearson_r=pearson_r, spearman=spearman, bias=bias, MAE=mae, RMSE=rmse))

# 4) Scatter (sample for speed if needed)
set.seed(1)
df_plot <- if (nrow(df) > 1e6) df[sample.int(nrow(df), 1e6), ] else df

ggplot(df_plot, aes(ref30_h, mod_h)) +
  geom_hex(bins = 40) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  labs(
    x = "Reference (5 m → 30 m mean)",
    y = "Model (30 m)",
    title = sprintf("Agreement at 14:00 — r = %.2f, RMSE = %.2f", pearson_r, rmse)
  ) +
  coord_equal()
...
# Compute shared breaks and color scale limits
mm_mod <- terra::minmax(mod_h)
mm_ref <- terra::minmax(ref30_h)
rng <- range(c(mm_mod, mm_ref), na.rm = TRUE)

# ------------------------------------------------
# Convert shapefiles to sf objects if not already
# ------------------------------------------------
...
p1 <- ggplot() +
  ...
  scale_fill_viridis_c(name = "°C", limits = rng) +
  coord_sf() +   # <<-- use coord_sf instead of coord_equal
  labs(title = "RK model") +
...
combined_plot <- (p1 + p2 + 
  ...
  plot_annotation(
    title = "Comparison of RK Model and Reference Temperature Maps — 14:00",
    theme = theme(
...
# ggsave("rk_vs_ref_14-00.png", combined_plot, width = 10, height = 5, dpi = 300)

```


```{r}
# 1) How many residuals are negative/positive?
resid <- df$rk_pred - df$`2_22_074_ist_t14_rev01`
summary(resid)
mean(resid); median(resid)
mean(resid < 0); mean(resid > 0)  # share of negatives/positives

# 2) Any unit mismatch or scaling oddities?
summary(df$rk_pred); summary(df$`2_22_074_ist_t14_rev01`)
# If values are ~280–320 it's Kelvin; ~10–45 is °C.
# A 273.15 shift would show up immediately.

# 3) Are we definitely evaluating at the same spatial support?
# (5 m aggregated to model’s 30 m) – you already did this, just re-assert:
stopifnot(terra::same.crs(ref30, mod))
...
ggplot(data.frame(resid), aes(resid)) +
  ...
  labs(x = "Model − Reference (°C)", y = "Count",
       title = "Residuals distribution at 14:00")

# Residual map (diverging, centered at 0)
resid_r <- mod_h - ref30_h
rmax <- max(abs(terra::global(resid_r, "min", na.rm=TRUE)$min),
            abs(terra::global(resid_r, "max", na.rm=TRUE)$max))
plot(resid_r, zlim = c(-rmax, rmax), col = hcl.colors(21, "Blue-Red 3"),
     main = "Residuals (Model − Reference) at 14:00 [°C]")

resid_r <- mod_h - ref30_h
terra::global(resid_r, c("min","mean","max"), na.rm=TRUE)
sum(values(resid_r) < 0, na.rm = TRUE)  # should be 0 or near 0 if all positive
...
mdae        <- median(abs(resid), na.rm = TRUE)

# R² and regression line (does the model need an offset/slope correction?)
fit <- lm(df$`2_22_074_ist_t14_rev01` ~ df$rk_pred)
summary(fit)$coefficients   # intercept/slope
summary(fit)$r.squared

# Normalized errors (help compare across days)
rng <- diff(range(df$`2_22_074_ist_t14_rev01`, na.rm = TRUE))
nrmse <- rmse / rng

```

```{r}
# Ensure CRS match between rasters and vector
...
# Add extracted values to landuse polygons
...
# Apply to your plot
p <- ggplot(landuse_summary, aes(x = ref_mean, y = rk_mean, color = class_2018)) +
  geom_point(alpha = 0.7, size = 2) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "black") +
  scale_color_manual(values = landcover_colors, name = "Landcover Class") +
  coord_equal() +
  labs(
    title = "RK vs Reference Mean Temperature by Landcover Class (09 July 2023)",
    x = "Reference Mean Temperature (°C)",
    y = "RK Mean Temperature (°C)"
  ) +
  theme_minimal(base_size = 13) +
  theme(
  ...
  guides(color = guide_legend(ncol = 3))  # multi-column legend
...
p_corr <- ggplot(cor_summary, aes(
  ...
  geom_col() +
  coord_flip() +
  scale_fill_manual(values = landcover_colors, name = "Landcover Class") +
  labs(
    title = "Correlation Between RK and Reference Temperatures by Landcover Class",
    subtitle = "Pearson’s r computed per landcover category (n varies per class)",
    caption = "Data: RK vs Reference mean temperature (09 July 2023)",
    x = "Landcover Class (class_2018)",
    y = "Pearson Correlation (r)"
  ) +
  theme_minimal(base_size = 13) +
  theme(
    legend.position = "none",  # Hide legend (colors already explained elsewhere)
    plot.title = element_text(hjust = 0.5, face = "bold"),
...
```{r}
ggplot(cor_summary, aes(
  x = reorder(class_2018, corr),
  y = corr,
  fill = class_2018
)) +
  geom_col(aes(alpha = abs(corr)), width = 0.7, color = "grey30") +
  scale_fill_manual(values = landcover_colors) +
  scale_alpha_continuous(range = c(0.6, 1)) +
  coord_flip() +
  labs(
    title = "Correlation Between RK Model and Reference Temperatures by Landcover Class (09 July 2023 at 2 pm)",
    subtitle = "Pearson’s r computed per landcover category (n varies per class)",
    caption = "Data: RK model output, Urban Climate Analysis Heidelberg (2023),\nUrban Atlas Land Cover/Land Use (2018), Europe, 6-yearly",
    x = NULL,
    y = expression("Pearson Correlation (r)")
  ) +
  theme_minimal(base_size = 13) +
  theme(
