...
# Ensure 'geom' is the active geometry column
stations <- temp_clean_station_hm
sf::st_geometry(stations) <- "geom"

# Keep one unique geometry per entity_id
stations_1row <- stations %>%
  dplyr::select(entity_id, geom) %>%
  dplyr::distinct(entity_id, .keep_all = TRUE)


# 3) Extract coordinates
cc <- sf::st_coordinates(stations_1row)

station_df <- stations_1row %>%
  sf::st_drop_geometry() %>%
  mutate(X = cc[, 1], Y = cc[, 2]) %>%
  select(entity_id, X, Y)

# (Optional) Convert to 'sp' and match CRS to your grid object
station_sp <- station_df
coordinates(station_sp) <- ~ X + Y
proj4string(station_sp) <- CRS(proj4string(grids_sp))
...
timestamps <- unique(temp_clean_station_hm$chour)[1:24]  
...
# Read CSV
cv_metrics <- read.csv("../rk_variogram_test/cross_validation_metrics.csv")
...
rk_metrics <- read.csv("../rk_variogram_test/rk_metrics.csv")
...
lm_metrics <- read.csv("../rk_variogram_test/lm_model_performance_summary.csv")
...
# Save plot
# ggsave("../plots/lm_metrics_by_day.png", plot = p_lm, width = 12, height = 8, dpi = 300)
```
