...
temp <- sf::read_sf('../data/temp_HD/sensor_data_20230105_31days.gpkg')
...
# ★ CHANGED: create a local-hour bucket (DST-safe) and then a stable integer key (UTC hours)
temp <- temp %>%
  mutate(
    hour_local = floor_date(dateobserved, unit = "hour"),                       
    chour      = as.integer(as.numeric(with_tz(hour_local, "UTC")) / 3600)      
  )

# Calculate hour count since epoch, rounded down to current hour
temp$chour <- floor(as.numeric(temp$dateobserved) / 3600)

# ★ CHANGED: test conversion using helper formatting (defined later). If running here, keep a simple check:
if (length(temp$chour) >= 66163) {
  print(format(as.POSIXct(temp$chour[66163] * 3600, origin = "1970-01-01", tz = tz_berlin), "%Y-%m-%d %H:%M:%S %Z"))
}
...
temp_hm <- temp %>%
  filter(month(dateobserved) == 7) %>% 
  group_by(stationname, hour_local, chour) %>%
  summarise(mh_temp = mean(temperature, na.rm = TRUE), .groups = "drop") %>%
  mutate(hour = hour_local)
...
station_coords <- locs %>%
  dplyr::distinct(stationname, .keep_all = TRUE) %>%
  dplyr::select(stationname, X, Y)
...
output_dir <- "../rk_results_testweek"
...
timestamps <- unique(temp_hm$chour)[1:168]  # Test with first 3 unique chour timestamps
...
for (i in seq_along(timestamps)) {                           
  ...
  p_all <- ggplot(all_temp_data, aes(x = hour, y = mh_temp, color = stationname, group = stationname)) +  
    geom_line() +
    geom_point(size = 1) +
  ...
  print(
    plot(rv.temp, rvgm.temp,
         main = paste0("Directional Variogram Fit (0° vs 90°)\n", dt_lab),   
...
Plot to analyse model performance metrices

```{r}
# Read CSV
cv_metrics <- read.csv("../rk_results/cross_validation_metrics.csv")

# Reshape to long format for easy plotting
cv_metrics_long <- cv_metrics %>%
  pivot_longer(cols = c(cv_rmse, cv_mae, cv_bias),
               names_to = "metric",
               values_to = "value")

# Plot metrics over timestamp
ggplot(cv_metrics_long, aes(x = timestamp, y = value, color = metric)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  theme_minimal(base_size = 14) +
  labs(title = "Cross-Validation Metrics over Time",
       x = "Timestamp",
       y = "Value",
       color = "Metric") +
