...
# Compute Pearson correlation per covariate
cors <- station_long %>%
  group_by(covariate) %>%
  summarise(
    r = cor(value, mean_temp, method = "spearman", use = "pairwise.complete.obs")
  ) %>%
  mutate(
    label = sprintf("italic(r) == %.2f", r)   # format for plotmath
  )

# Plot correlation + Spearman
p_corr_temp_cov <- ggplot(station_long, aes(x = value, y = mean_temp)) +
  ...
  geom_text(data = cors, aes(label = label), x = -Inf, y = Inf, hjust = -0.2, 
            vjust = 2, parse = TRUE, size = 5, fontface = "bold",
            inherit.aes = FALSE) +
...
ggsave("../plots/correlation_temperature_vs_covariates.png", 
...
p_vif <- ggplot(vif_df, aes(x = covariate, y = VIF, fill = covariate)) +
  ...
  annotate("text", x = 0.5, y = 5.2, label = "VIF > 5 high multicollinearity",
           hjust = 0, size = 4, color = "grey40") +
  geom_text(aes(label = round(VIF, 2)),
          vjust = -0.5, size = 5) +
  labs(title = "Variance Inflation Factors (VIF) for Model Covariates",
