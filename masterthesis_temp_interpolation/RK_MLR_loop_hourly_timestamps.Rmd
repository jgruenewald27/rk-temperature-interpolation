  ...
  if (moran_p >= 0.05) {
      ...
      rvgm.temp <- vgm(
        ...
        range = 0
          ...
          ## Create dummy variogram for plotting
    rv.temp <- variogram(
    ...
    if (anis_ratio > anis_threshold) {
      ...
      cand_models <- list(
        ...
        vgm(psill0, "Gau", min(dir_ranges), nug0, anis = c(anis_dir, anis_ratio))
      )
      model_labels <- c("Exp (anis)", "Sph (anis)", "Gau (anis)")
      ...
      cand_models <- list(
        ...
        vgm(psill0, "Gau", range0, nug0)
      )
      model_labels <- c("Exp", "Sph", "Gau")
    }
    ...
    if (!inherits(rvgm.temp, "variogramModel")) {
      cat("    WARNING: All model fits failed — reverting to default exponential model.\n")
      rvgm.temp <- cand_models[[1]]
    
    } else {
    
      rv_range <- rvgm.temp$range
      rv_psill <- rvgm.temp$psill
