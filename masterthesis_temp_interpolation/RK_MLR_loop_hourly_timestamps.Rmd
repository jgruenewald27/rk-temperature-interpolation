...
for (i in seq_along(timestamps)) {
  ...
  if (moran_p >= 0.05) {
    ...
    ## Candidate theoretical models (Oliver 2015)
    cand_models <- list(
      vgm(psill = psill0, model = "Exp", range = range0, nugget = nug0),
      vgm(psill = psill0, model = "Sph", range = range0, nugget = nug0),
      vgm(psill = psill0, model = "Gau", range = range0, nugget = nug0)
    )
    
    cat("    Trying candidate models: Exp, Sph, Gau…\n")
    ...
    ## Extract SSErr values where models succeeded
    ...
    cat(sprintf("    → Selected model: %s (SSErr = %.3f)\n",
                cand_models[[best_idx]]$model[2], sse_vals[best_idx]))

    if (!inherits(rvgm.temp, "variogramModel")) {
        cat("    WARNING: All model fits failed — using default exponential model.\n")
        rvgm.temp <- cand_models[[1]]
      } else {
        ...
        if (bad_model) {
          cat("    WARNING: Fitted model invalid — reverting to default exponential model.\n")
          rvgm.temp <- cand_models[[1]]
        }
      }
    }
