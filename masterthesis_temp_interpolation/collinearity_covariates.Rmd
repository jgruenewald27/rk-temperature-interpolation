...
```{r Load and pre-process environmental covariates}
...
```{r Create a Raster Stack for all the covariate information}
...
```{r Add lat and lon to the raster stack}
...
```{r Stack the three raster layer}
...
UNDERSTAND IF RESIDUALS ARE SPATIALLY AUTOCORRELATED OR NOT -> OF GREAT IMPORTANCE TO 
THE FURTHER APPROACH

CALCULATE THIS FOR THE LINEAR AND NON-LINEAR MODEL

```{r Calculate Morans I on regression residuals for single day}

# Pick one day
one_day <- as.Date("2023-07-07")

# Filter to that day (keep sf)
df_day <- temp_clean_station_hm_cov %>%
  filter(as.Date(hour) == one_day) %>%
  st_as_sf()

# Loop across each timestamp
moran_results <- df_day %>%
  ...
  map_dfr(function(df_slice) {
    ...
    # asymptotic test
    mt <- moran.test(df_slice$residuals, lw, zero.policy = TRUE)
    
    # Monte Carlo test
    mc <- moran.mc(df_slice$residuals, lw, nsim = 999, zero.policy = TRUE)
    
    # 95% envelope from permutation distribution
...
p_moran <- ggplot(moran_results, aes(x = hour, y = moran_i, group = 1)) +
  geom_ribbon(aes(ymin = lo95, ymax = hi95), fill = "grey80", alpha = 0.5) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_hline(yintercept = unique(moran_results$expected), linetype = "dotted") +
  geom_line(color = "black") +
  geom_point(aes(color = p_mc < 0.05), size = 2) +
  labs(
    title = "Moran's I of Regression Residuals",
    subtitle = "Dashed = 0; dotted = expectation under null; grey band = 95% envelope",
    x = "Hour of Day",
    y = "Moran's I",
    color = "Permutation Test (p < 0.05)"
  ) +
  theme_minimal() +
  theme(
...
# save as PNG
#ggsave("../plots/moransI_residuals_2023-07-07.png", p_moran, width = 8, height = 5, dpi = 300)
```

```{r Loop to calculate Morans I on regression residuals}

# Define the month you want to process
...
for (one_day in all_days) {
  ...
  # Skip if no data for that day
  if (nrow(df_day) == 0) next

  # Run Moran’s I at each timestamp
  moran_results <- df_day %>%
    group_split(hour) %>%
    map_dfr(function(df_slice) {
  ...
  p_moran <- ggplot(moran_results, aes(x = hour, y = moran_i, group = 1)) +
    ...
    labs(
      title = "Moran's I of Regression Residuals",
      subtitle = paste("Day:", one_day, "| Dashed = 0; dotted = expectation; grey band = 95% envelope"),
      x = "Hour of Day",
      y = "Moran's I",
      color = "Permutation Test (p < 0.05)"
    ) +
  ...
  out_file <- paste0("../plots/moransI_residuals/moransI_residuals_", one_day, ".png")
...
```{r}
# --- 1) Quick diagnostic panel (includes residuals vs fitted & scale-location) ---
par(mfrow = c(2, 2))
plot(mlr_coll)          # 1: Residuals vs Fitted, 2: QQ, 3: Scale-Location, 4: Residuals vs Leverage
par(mfrow = c(1, 1))

# --- 2) Focused residuals vs fitted (studentized) ---
res_std <- rstudent(mlr_coll)
fit     <- fitted(mlr_coll)
plot(fit, res_std, pch=20, xlab="Fitted values", ylab="Studentized residuals")
abline(h = 0, col = "red", lwd = 2)
# add a smooth to spot patterns
lines(lowess(fit, res_std), col = "blue", lwd = 2)

# --- 3) Formal homoscedasticity tests ---
library(lmtest)
library(car)

bptest(mlr_coll)                               # Breusch–Pagan (generic heteroskedasticity)
ncvTest(mlr_coll)                              # Car's non-constant variance score test
# White-style test (variance depends on fitted & fitted^2)
bptest(mlr_coll, ~ fit + I(fit^2))
```
