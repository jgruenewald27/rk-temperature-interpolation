...
Calculate correlation between the three covariates and the dependent variable temperature
...
# Scatter plots
ggplot(station_df, aes(x = elevation, y = mean_temp)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = TRUE, color = "red") +
  labs(title = "Temperature vs Elevation",
       subtitle = "Yearly mean per station",
       x = "Elevation [m]",
       y = "Mean Temperature [°C]")

ggplot(station_df, aes(x = canopy_height, y = mean_temp)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = TRUE, color = "blue") +
  labs(title = "Temperature vs Canopy Height",
       subtitle = "Yearly mean per station",
       x = "Elevation (m)",
       y = "Mean Temperature (°C)")

ggplot(station_df, aes(x = building_height, y = mean_temp)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = TRUE, color = "darkgreen") +
  labs(title = "Temperature vs Building Height",
       subtitle = "Yearly mean per station")
```
...
```{r Calculate Morans I on regression residuals}

library(sf)

# Get geometry from lat and lon column and reproject to project
sf_pts <- st_as_sf(temp_clean_station_hm_cov, coords = c("lon", "lat"), crs = 4326)
sf_pts <- st_transform(sf_pts, 32632)
crs(sf_pts)

# 2) Residuals from your fitted model (same row order!)
resid_lm <- residuals(mlr_coll)

# 3) k-nearest neighbors (k = 8) and Moran's I
knn <- knearneigh(xy, k = 8)
nb  <- knn2nb(knn)
lw  <- nb2listw(nb, style = "W", zero.policy = TRUE)

moran.test(resid_lm, lw, zero.policy = TRUE)          # asymptotic test
moran.mc(resid_lm, lw, nsim = 999, zero.policy = TRUE) # permutation test
