...
p_ref_map <- ggplot() +
  ...
  geom_sf(data = hd_bounds_sf, fill = NA, color = "black", linewidth = 0.4) +
  scale_fill_viridis_c(name = "Physiologically Equivalent\nTemperature (PET) [°C]", limits = rng) +
  coord_sf() +
  annotation_scale(location = "br", width_hint = 0.3) +
  annotation_north_arrow(location = "tl", which_north = "true", width = unit(0.9, "cm"),
                         style = north_arrow_orienteering) +
...
resid_var_land <- ggplot(
  res_var_summary,
  aes(x = reorder(lu_label, mean_sd), y = mean_sd, fill = lu_label)) +
  geom_col(width = 0.6, color = "grey30") +
  geom_text(aes(label = sprintf("%.2f", mean_sd)), hjust = -0.1, size = 3) +
  scale_fill_manual(values = landcover_colors) +
  coord_flip() +
  labs(x = NULL, y = "Mean SD (°C)") +
  theme_bw(base_size = 13) +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold"),
    plot.subtitle = element_text(hjust = 0.5),
...
# Save as PNG
ggsave("../plots_report/residual_var_relative_per_landcover.png", 
       resid_var_land, width = 10, height = 5, dpi = 300)
```

```{r}
# Option 2: Cleveland dot plot with tight x-axis (+ overall mean reference line)
# Assumes you already have `res_var_summary` with columns: lu_label, mean_sd
...
# Save as PNG
ggsave("../plots_report/temporal_sd_hourly_rk_predictions.png", 
       temp_var, width = 10, height = 5, dpi = 300)
```
...
# Save as PNG
ggsave("../plots_report/temporal_cv_hourly_rk_predictions.png", 
       temp_coeff_var, width = 10, height = 5, dpi = 300)
```
