...
```{r}
# ---------------------------------------------------------------
# Loop to calculate Moran's I on regression residuals (multi-k)
# ---------------------------------------------------------------
...
# ---------------------------------------------------------------
# MAIN LOOP — DAY BY DAY, HOUR BY HOUR
# ---------------------------------------------------------------
for (one_day in all_days) {
  message("Processing ", one_day)

  df_day <- mlr_residuals_sf %>% filter(day == one_day)
  if (nrow(df_day) == 0) next

  # --- Compute Moran's I for each hour ---
  moran_results <- df_day %>%
    group_split(hour) %>%
    map_dfr(function(df_slice) {
      ...
      results_k <- lapply(k_values, function(k) {
        ...
        # Moran’s I (MC version preferred)
        mt <- moran.test(df_slice$residual, lw, zero.policy = TRUE)
        mc <- moran.mc(df_slice$residual, lw, nsim = 999, zero.policy = TRUE)
  ...
  moran_results_clean <- moran_results %>%
    ...
    select(date, hour, k, moran_i, p_mc, moran_sig)

  moran_summary_list[[as.character(one_day)]] <- moran_results_clean
}

# Combine all results
moran_summary <- bind_rows(moran_summary_list)
