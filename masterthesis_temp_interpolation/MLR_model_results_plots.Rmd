...
# Save each page
ggsave(filename = file.path(output_plots, sprintf("covariate_importance.png")),
...
station_coords_df <- station_coords_sf %>%
  st_transform(32632) %>%                     # ensure projected CRS for Moran's I
  mutate(
    lon = st_coordinates(.)[, 1],             # UTM easting
    lat = st_coordinates(.)[, 2]              # UTM northing
  ) %>%
  st_drop_geometry()
...
# Save each page
ggsave(filename = file.path(output_pval, sprintf("covariate_pvalues.png")), plot = p_pval,
