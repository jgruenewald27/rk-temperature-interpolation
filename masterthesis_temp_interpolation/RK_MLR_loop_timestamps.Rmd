...
# Read MLR metrics
lm_metrics <- read.csv("../RK_07_2023_12days/lm_model_performance_summary.csv",
...
preprocess_metrics <- function(df) {
  df %>%
    mutate(
      # If the timestamp is only "YYYY-MM-DD", append "00:00:00"
...
# Daily Evaluation of the LOOCV RK Metrics
p_loocv <- ggplot(rk_cv_metrics_long, aes(x = hour, y = value, color = metric, group = metric)) +
    geom_line(linewidth = 1.2) +
    ...
    labs(
      title    = "Daily Variation Regression-Kriging LOOCV Performance",
      subtitle = "Hourly RMSE and MAE from leave-one-out cross-validation of hourly temperature models",
      x        = "Hour of the Day",
      y        = "Error Metric [°C]",
      color    = "Metrics"
    ) +
...
# Save plot
ggsave("../plots_report/LOOCV_RK_metrics_12days.png", 
...
weather_vars <- c(
  "temperature_mean", "relativehumidity_mean", "dewpoint_mean",
...
rel_rmse_weather <- ggplot(df_long, aes(x = value, y = rk_cv_rmse)) +
  ...
  theme_minimal(base_size = 14) +
  labs(
    title = "Relationship Between RK LOOCV RMSE and Hourly Mean Weather Variables",
    subtitle = "Hourly means (1–12 July 2023) compared against RK model error",
...
ggsave(
  filename = file.path("../plots/hourly_weather_relationship_rmse.png"),
...
di_weather <- ggplot(df_long, aes(
  ...
  theme_minimal(base_size = 14) +
  labs(
    title = "Diurnal Patterns of Hourly Mean Weather Variables and Their Influence on RK Model Performance",
    subtitle = "Hourly means (1–12 July 2023) with LOOCV RK RMSE to highlight performance variation across the diurnal cycle",
    x = "Hour of the Day",
...
# Save the output plot
ggsave(
  filename = file.path("../plots/hourly_weather_rk_loocv_rmse.png"),
  plot = di_weather,
  width = 12, height = 8, dpi = 300
)
```
...
p_ref_map <- ggplot() +
  ...
  annotation_north_arrow(location = "tl", which_north = "true", width = unit(0.9, "cm"),
                         style = north_arrow_orienteering) +
  labs(title = "Urban Climate Analysis Heidelberg",
       subtitle = sprintf("PET value range: %.1f – %.1f °C", 
...
p_resid_hist <- ggplot(data.frame(resid), aes(x = resid)) +
  ...
  labs(
    title = "Residual Distribution (RK Model − Urban Climate Analysis HD, 09-07-2023 14:00)",
...
p_resid_map <- ggplot() +
  ...
  annotation_north_arrow(location = "tl", which_north = "true", width = unit(0.9, "cm"),
                         style = north_arrow_orienteering) +
  labs(
    title = "Spatial Distribution of Residuals (RK Model − Urban Climate Analysis HD) on 09-07-2023 at 14:00 [°C]",
