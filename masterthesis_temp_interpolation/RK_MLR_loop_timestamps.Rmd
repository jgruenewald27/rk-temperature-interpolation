...
# Save plot
ggsave("../plots_report/MLR_RK_metrics_12days.png", plot = p_lm, width = 12, height = 8, dpi = 300)
```

```{r Analyse LOOCV metrics 12 days}

# Daily Evaluation of the LOOCV RK Metrics
p_cv <- ggplot(rk_cv_metrics_long, aes(x = hour, y = value, color = metric, group = metric)) +
    geom_line(linewidth = 1.2) +
...
cv_weather_combined <- cv_metrics_join %>%
  left_join(
    df_hourly_means,
    by = c("hour_ts" = "hour")
  )
