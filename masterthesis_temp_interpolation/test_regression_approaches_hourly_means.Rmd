...
output_dir <- "../regression_test_residuals"
...
for (i in seq_len(n_pages)) {
  p_cov <- ggplot(coef_all, aes(x = hour)) +
    geom_area(aes(y = lmg, fill = term), position = "fill", alpha = 0.95) +
    geom_line(aes(y = r_squared, color = "Model R²"), linewidth = 0.6) +
    ggforce::facet_wrap_paginate(~ date, ncol = 3, nrow = 4, page = i) +
    scale_x_continuous(breaks = seq(0, 24, 6), limits = c(0, 24)) +
    scale_y_continuous(
      name = "Covariate Importance (LMG, % of R²)",
    ...
    labs(
      title    = sprintf("Daily Variation of Covariate Importance (%d/%d)", i, n_pages),
...
for (i in seq_len(n_pages)) {
  ...
  p_pval <- ggplot(coef_all, aes(x = hour, y = term, fill = p_category)) +
    ...
    values = c(
      "< 0.001" = "#440154",       # dark purple
      "< 0.01"  = "#31688e",       # blue
      "< 0.05"  = "#35b779",       # green
      "≥ 0.05 (n.s.)" = "grey85"   # light grey
    ),
    breaks = c("< 0.001", "< 0.01", "< 0.05", "≥ 0.05 (n.s.)")
    ) +
    labs(
      x = "Hour of Day",
      y = "Covariate",
