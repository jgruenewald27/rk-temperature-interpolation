...
```{r}
# Create list of index positions for the first week of each month
index_by_month <- lapply(1:12, function(m) {
  which(month(temp_clean_station_hm$hour) == m & day(temp_clean_station_hm$hour) <= 7)
})

# Name each list element by month
names(index_by_month) <- month.name

# Print all indices clearly
for (m in names(index_by_month)) {
  cat("\n", m, "indices (first week):\n")
  print(index_by_month[[m]])
}
```
...
end_date   <- as.POSIXct("2023-07-10 23:00:00", tz = "Europe/Berlin")
...
```{r}
ggplot(fit, aes(as.POSIXct(timestamp, tz="Europe/Berlin"), adj_r_squared)) +
  geom_line() + geom_point(size=1) +
  labs(x="Zeitstempel", y="adj. R²", title="Modellgüte über die Zeit") +
  theme_minimal(12)

ggplot(coef_all, aes(as.POSIXct(timestamp, tz="Europe/Berlin"), beta_std, color=term)) +
  geom_hline(yintercept=0, linetype="dashed") +
  geom_line() + geom_point(size=0.9) +
  labs(x="Zeitstempel", y="β (standardisiert)",
       title="Einflussstärke der Kovariaten über die Zeit") +
  theme_minimal(12)

ggplot(coef_all, aes(as.POSIXct(timestamp, tz="Europe/Berlin"), term, fill=beta_std)) +
  geom_tile() +
  scale_fill_gradient2(name="β") +
  labs(x="Zeitstempel", y="Kovariate", title="Heatmap der standardisierten Effekte") +
  theme_minimal(12)

ggplot(coef_all, aes(as.POSIXct(timestamp, tz="Europe/Berlin"), lmg, fill=term)) +
  geom_area(position="fill") +
  labs(x="Zeitstempel", y="Anteil an R² (LMG)",
       title="Relative Wichtigkeit der Kovariaten (sum=1)") +
  theme_minimal(12)
```
...
# Import z-normalized MLR regression model metrics
coef_all <- read.csv(coef_file_z)
fit <- read.csv(metrics_file_z)

# sicherstellen, dass timestamp gleich formatiert ist wie in coef_all
fit <- fit %>%
  mutate(
    timestamp = if_else(
      nchar(timestamp) == 10,
    ...
    timestamp = as.POSIXct(timestamp, tz = "Europe/Berlin")
  )
...
# Make sure you have roughly 10 unique days and 24 hours per day
# length(unique(coef_all$date))


# Plot daily variation of covariates importance
ggplot(coef_all, aes(x = hour, y = lmg, fill = term)) +
  geom_area(position = "fill", alpha = 0.95) +
  geom_line(aes(y = r_squared), color = "black", linewidth = 0.8) +
  facet_wrap(~ date, ncol = 2) +
  scale_x_continuous(breaks = seq(0, 24, 6), limits = c(0, 24)) +

  # left axis = share in %, right axis = R² (0–1)
  scale_y_continuous(
    name = "Share of R² (LMG, Sum = 1)",
  ...
  scale_fill_manual(
    name = "Covariate",
    values = c(
      "building_height_z" = "#E69F00",  # orange
      "canopy_height_z"   = "#009E73",  # teal
      "elevation_z"       = "#56B4E9"   # blue
    ),
    labels = c(
      "building_height_z" = "Building height (z)",
      "canopy_height_z"   = "Canopy height (z)",
  ...
  labs(
    ...
    title = "Daily Variation of Covariate Importance",
    subtitle = "z-standardized covariates within each hour across all stations\nRelative contribution of static environmental covariates to hourly mean temperature prediction"
  ) +
  theme_minimal(base_size = 14) +
  theme(
...
coef_sig <- coef_all |>
  dplyr::group_by(term) |>
  dplyr::mutate(p_bh = p.adjust(p_robust, method="BH"),
                sig  = p_bh < 0.05) |>
  dplyr::ungroup()

ggplot(coef_sig, aes(as.POSIXct(timestamp, tz="Europe/Berlin"), term, fill=sig)) +
  geom_tile() +
  scale_fill_manual(values=c("TRUE"="black","FALSE"="grey85"), name="Signifikant (BH<0.05)") +
  labs(x="Zeitstempel", y="Kovariate", title="Signifikanz über die Zeit") +
  theme_minimal(12)

ggplot(coef_sig, aes(as.POSIXct(timestamp, tz="Europe/Berlin"), term, fill=beta_std)) +
  geom_tile() +
  geom_point(data=dplyr::filter(coef_sig, sig), shape=21, size=1.5, color="black") +
  scale_fill_gradient2(name="β") +
  labs(x="Zeitstempel", y="Kovariate",
       title="Effektstärke (β) mit Signifikanz-Markern") +
  theme_minimal(12)

top_hours <- coef_sig |>
  dplyr::group_by(term) |>
  dplyr::slice_max(order_by=abs(beta_std), n=5, with_ties=FALSE) |>
  dplyr::ungroup()

ggplot(top_hours,
       aes(x=reorder(format(as.POSIXct(timestamp, tz="Europe/Berlin"), "%d.%m %H:%M"),
                     abs(beta_std)),
           y=beta_std, color=sig)) +
  geom_segment(aes(xend=..x.., y=0, yend=beta_std)) +
  geom_point(size=2) +
  facet_wrap(~term, scales="free_y") +
  coord_flip() +
  scale_color_manual(values=c("TRUE"="black","FALSE"="grey60"), name="BH<0.05") +
  labs(x="Timestamp", y="β", title="Stärkste Zeitpunkte pro Kovariate") +
  theme_minimal(12)
