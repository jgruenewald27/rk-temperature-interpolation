...
end_date   <- as.POSIXct("2023-07-31 23:00:00", tz = "Europe/Berlin")
...
# Format timestamps in both data frames
fit <- fit %>%
  mutate(
    timestamp = if_else(
      nchar(timestamp) == 10,
...
for (i in seq_len(n_pages)) {
  ...
  p_cov <- ggplot(coef_all, aes(x = hour, fill = term)) +
    geom_area(aes(y = lmg), position = "fill", alpha = 0.95) +
    ...
    labs(
      title    = sprintf("Daily Variation of Covariate Importance (Page %d/%d)", i, n_pages),
      subtitle = "Left axis: relative contribution of covariates (LMG, % of R²)\nRight axis: total model R² based on hourly MLR with z-standardized covariates",
      x = "Hour of Day",
  ...
  # Save each page
  ggsave(
    filename = file.path(output_plots, sprintf("covariate_importance_page_%02d.png", i)),
    plot = p_cov,
...
for (i in seq_len(n_pages)) {
  ...
  p_pval <- ggplot(coef_all, aes(x = hour, y = term, fill = p_robust)) +
    ...
    labs(
      x = "Hour of Day",
      y = "Covariate",
      title = sprintf("Hourly Significance of Covariates (Page %d/%d)", i, n_pages),
      subtitle = "Darker colors indicate higher significance (lower p-values)\nGrey cells: non-significant (p ≥ 0.05)"
    ) +
  ...
  # Save each page
  ggsave(
    filename = file.path(output_pval, sprintf("covariate_pvalues_page_%02d.png", i)),
    plot = p_pval,
    width = 12, height = 10, dpi = 300
  )
}
