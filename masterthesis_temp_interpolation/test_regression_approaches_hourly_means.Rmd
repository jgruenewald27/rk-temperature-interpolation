...
coef_all <- coef_all %>%
  mutate(
    timestamp = if_else(
      nchar(timestamp) == 10,             # check if timestamp is 10 characters
      paste0(timestamp, " 00:00:00"),     # add midnight time if 10 and not more
      timestamp                                            
    ),
...
ggplot(coef_all, aes(x = hour, y = lmg, fill = term)) +
  ...
  labs(
    ...
    title = "Daily Variation of Covariate Importance (LMG)",
    fill = "Covariate"
  ) +
