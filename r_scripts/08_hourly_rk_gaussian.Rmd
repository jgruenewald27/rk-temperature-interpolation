...
This just runs the entire hourly RK workflow with just the gaussian smoothed
canopy height data instead of the original one.
...
w <- focalWeight(tch_aligned, d = 22.86647, type = "Gauss")
...
output_dir <- "../gaussian_ANBH_MLR_07_2023_12days"
...
lm_metrics <- read.csv("../gaussian_ANBH_MLR_07_2023_12days/lm_metrics/lm_model_performance_summary.csv")
  ...
  p_cov <- ggplot(coef_all, aes(x = hour)) +
    ...
    scale_fill_manual(
      ...
      labels = c(
        ...
        "canopy_cooling_z"   = "Gaussian-smoothed Canopy Height",
...
mlr_residuals <- read.csv(
  "../gaussian_ANBH_MLR_07_2023_12days/mlr_residuals_all.csv",
...
for (i in seq_len(n_pages)) {
  p_pval <- ggplot(coef_all, aes(x = hour, y = term, fill = p_category)) +
    ...
    scale_y_discrete(labels = c(
      "building_height_z" = "Building Height",
      "canopy_cooling_z"   = "Gaussian-smoothed\nCanopy Height",
      "elevation_z"       = "Elevation"
    )) +
    scale_x_continuous(breaks = seq(0, 24, 6), limits = c(0, 24)) +
    scale_fill_manual(
...
# ggsave(filename = file.path("../plots_report/gaussian_sig_covariates_pval.png"), plot = p_pval,
#        width = 12, height = 10, dpi = 300)
```
