---
title: "RK_gam_different_timestamps"
...
for (ts in timestamps) {
  ...
  # Fit GAM model
  gam_all_mm <- gam(mh_temp ~ elev + canopy + bldg, data = temp_shm.ex)
  summary_gam <- summary(gam_all_mm)
  
  # Extract model metrics for GAM
  # MODEL EVALUATION GAM IS MISSING IN CURRENT OUTPUTS
  
  # Append to tracking data frame
  if (!exists("gam_metrics_all")) {
    gam_metrics_all <- model_metrics
  } else {
    gam_metrics_all <- rbind(gam_metrics_all, model_metrics)
  }

  # Save model's residuals back into original df as new column
  temp_shm.ex$residuals <- gam_all_mm$residuals
...
metrics_file <- file.path(output_dir, "gam_model_performance_summary.csv")
write.csv(gam_metrics_all, metrics_file, row.names = FALSE)
