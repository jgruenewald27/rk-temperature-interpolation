...
lzn.vgm <- variogram(log(mean_temperature) ~ 1, july_mean_temp, width = 0.5, cutoff = 10) 

# Plot the sample variogram
#plot(lzn.vgm)

# Fit an exponential variogram model to the sample variogram
lzn.fit <- fit.variogram(lzn.vgm, model = vgm(1.5, "Exp", 5, 0.2))
...
lzn.kriged <- krige(log(norm_temperature) ~ 1, july_mean_temp, grid_points, model=lzn.fit)
```
