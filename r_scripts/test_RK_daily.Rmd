...
temp_dm <- temp %>%
  ...
  mutate(date = as.Date(dateobserved)) %>%  # Extract date (drop time)
  group_by(stationname, date) %>%  # Group by station and day
...
```{r}
temp_dm <- st_transform(temp_dm, crs="EPSG:32632")
st_crs(temp_dm)
```
...
dem <- terra::project(dem, y = crs(temp_dm))
...
# Reproject the elevation data to match correct CRS
tch <- terra::project(tch, y = crs(temp_dm))
...
# Reproject the elevation data to match correct CRS
ghs_bh <- terra::project(ghs_bh, y = crs(temp_dm))
