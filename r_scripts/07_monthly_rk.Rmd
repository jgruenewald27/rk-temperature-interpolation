...
- Computes monthly mean temperatures (2023) per station and runs a monthly 
RK loop (MLR trend + residual OK) with Moran’s I screening, variogram fitting, 
and LOOCV.
- Saves monthly performance summaries and variogram diagnostics.
- Produces time-series plots of monthly R², LOOCV RMSE/MAE, and RK RMSE/MAE.

--------------------------------------------------------------------------------

```{r Load and preprocess data}

# Complete data import and pre-processing of the temperature (Entire year 2023)
...
temp_monthly_mean_2023 <- temp_clean_station %>%
  ...
  summarise(mm_temp = mean(temperature, na.rm = TRUE),
...
# Create output dirs
output_dir <- "../Monthly_ANBH_RK_2023"
...
for (i in seq_along(timestamps)) {
  ...
  # Drop duplicates by lon + lat (keep first)
  extracted_vals <- extracted_vals |>
  dplyr::distinct(lon, lat, .keep_all = TRUE)
  
  # Bind safely
  temp_shm.ex <- dplyr::bind_cols(sel_temp_hm, extracted_vals)

  ## ------------------------------------------------
  ## STEP 2: Fit MLR
  ## ------------------------------------------------

  lm.temp_shm <- lm(mm_temp ~ elevation + canopy_height + building_height,
                    data = temp_shm.ex)
...
# Read LOOCV
lm_metrics <- read.csv("../Monthly_ANBH_RK_2023/mlr_model_performance_summary.csv",
                       stringsAsFactors = FALSE)
# Read the cv metrics
rk_cv_metrics <- read.csv("../Monthly_ANBH_RK_2023/rk_loocv_metrics.csv",
                       stringsAsFactors = FALSE)

# Read RK metrics
rk_metrics <- read.csv("../Monthly_ANBH_RK_2023/rk_metrics.csv",
                       stringsAsFactors = FALSE)
...
```{r Plot LOOCV monthly mean}

# Monthly Evaluation of the LOOCV
p_cv <- ggplot(rk_cv_metrics_long,
  ...
  labs(x = "Month", y = "Error Metric [°C]", color = "Metrics") +
...
# Save plot
ggsave("../plots_report/loocv_metrics_monthly_rmse_mae.png", plot = p_cv, width = 10, height = 6, dpi = 300)
```

```{r Plot RK metrics monthly mean}
...
p_rk <- ggplot(rk_metrics_long,
  ...
  scale_color_manual(
    values = c("rk_rmse" = "#0072B2", "rk_mae" = "#D55E00"),
...
```{r Plot MLR metrics monthly mean}

# Monthly MLR performance (R² & Adjusted R²)
p_lm <- ggplot(lm_metrics_long,
  ...
  scale_color_manual(
    values = c("r_squared" = "#0072B2", "adj_r_squared" = "#D55E00"),
  ...
  labs(x = "Month", y = "Metric Value", color = "Metrics") +
  theme_minimal(base_size = 14) +
  theme(
    legend.position  = "top",
    panel.grid.minor = element_blank(),
...
# Save plot
# ggsave("../plots_report/mlr_metrics_monthly_r2.png", plot = p_lm, width = 10, height = 6, dpi = 300)
```
